...
function initialize()
{
	...
	var enemy = new Enemy(580,32)
	enemies.push(enemy);

	var enemy = new Enemy(580,400)
	enemies.push(enemy);

	var enemy = new Enemy(32,400)
	enemies.push(enemy);
	...
	//Song "Root" accredited to Silent Partner in YouTube 'Create' section.
}
...
function drawCells(layer)
{
	for(var y = 0; y < MAP.th; y++)
	{
		for(var x = 0; x < MAP.tw; x++)
		{
			if (cells[layer][y][x] != 0)
			{
...
function runGame(deltaTime) 
{
	...
	for (var i = 0; i < objects.length; i++)
	{
    	var hit = intersects(
        	player.position.x, player.position.y,
        	player.width, player.height,
        	objects[i].x + 32,objects[i].y + 32,
			32,32)
		if (hit == true)
            {
				console.log("hit some food");
                scoreIncrease();
				objects.splice(i,1);
				return;
            }
...
function DrawUI()
{
	for (var i = 0; i < player.lives; ++i) 
	{
		...
		var y = SCREEN_HEIGHT - 80 - livesIcon.height;
		context.drawImage(livesIcon,x,y);
	}

	context.fillStyle = "black";
	context.font = "24px Impact";
	context.fillText("TIME:", 540, 555);
	context.fillStyle = "black";
	context.font="24px Impact";
	context.fillText(countUpTimer.toFixed(1).toString(), 595, 555);

	context.fillStyle = "black";
	context.font="24px Impact";
	context.fillText("SCORE:", 5, 555);
	context.fillStyle = "black";
	context.font="24px Impact";
	context.fillText(scoreCount.toFixed().toString(), 75, 555);
